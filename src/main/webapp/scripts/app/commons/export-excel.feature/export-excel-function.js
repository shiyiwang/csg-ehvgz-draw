(function() {
    define([
        ...
        return {
            ...
            // 适合只有选中数据后导出的模块
            exportSelectedToExcel: function (me){
                var selectedDataId = [],
                    selected = me.feature.views['grid:body'].components[0].getSelected();

                console.log('selected', selected);

                if (_.isArray(selected)) {
                    for (var i = 0; i < selected.length; i++) {
                        selectedDataId.push(selected[i].id);
                    }
                } else if (null != selected){
                    selectedDataId.push(selected.id);
                }
